...
class Schema
{
    ...
    function __toString()
    {
        return (string) json_encode($this->obj, JSON_THROW_ON_ERROR);
    }

    public static function toScript($yaml = '')
    {
        if (!$yaml) {
            return;
        }
        ...
        foreach ($array as $key => $properties) {
            $str .= "<script type=\"application/ld+json\">" . new self($key, $properties) . '</script>' . "\r\r";
        }
