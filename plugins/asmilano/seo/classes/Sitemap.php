...
class  Sitemap
{
    ...
    function generate()
    {
            ...
            if (class_exists($modelClass)) {
                ...
                foreach ($models as $model) {
                    if ($page->hasComponent('blogPost')) {
                        if (!(integer)$model->arcane_seo_options['enabled_in_sitemap']) {
                            continue;
                        }
        ...
        if (Helper::rainlabPagesExists()) {
            $staticPages = \Winter\Pages\Classes\Page::listInTheme(Theme::getActiveTheme());
            foreach ($staticPages as $staticPage) {
                if (!$staticPage->getViewBag()->property('enabled_in_sitemap')) {
                    continue;
                }
